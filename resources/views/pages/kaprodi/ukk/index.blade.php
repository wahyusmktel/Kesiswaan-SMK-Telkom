    ...
    <div class="py-6 w-full" x-data="ukkPage()" x-init="init()">
        <div class="w-full px-4 sm:px-6 lg:px-8">
            ...
            <div class="fixed top-5 right-5 z-[9999] flex flex-col gap-2 pointer-events-none" style="min-width:300px">
                <template x-for="toast in toasts" :key="toast.id">
                    <div class="pointer-events-auto flex items-start gap-3 px-4 py-3 rounded-xl shadow-2xl border text-sm font-semibold transition-all duration-300"
                        :class="{
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($ujians as $i => $ujian)
                                <tr class="bg-white hover:bg-gray-50/70 transition-colors">
                                    ...
                                    <td class="px-5 py-4 text-gray-700 max-w-[180px]">
                                        {{ $ujian->nama_project ?? '—' }}
                                    </td>
                                    ...
                                    <td class="px-5 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <button @click="openEdit({{ $ujian->id }},
        ...
        <template x-teleport="body">
            ...
            <div x-show="showModal"
                ...
                class="fixed z-[1001] flex items-start justify-center"
                :class="modalMaximized ? 'inset-0 p-0' : 'inset-0 p-4 pointer-events-none'"
                style="display:none"
                @mousemove.window="onDrag($event)"
                @mouseup.window="stopDrag()">

                <div class="bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden pointer-events-auto"
                    :class="modalMaximized
                    ...
                    <div class="flex items-center justify-between px-5 py-3.5 bg-gradient-to-r from-indigo-600 to-purple-700 text-white shrink-0 select-none"
                        ...
                        <div class="flex items-center gap-1.5">
                            ...
                            <button @click.stop="toggleMaximize()"
                                class="w-7 h-7 rounded-lg bg-white/15 hover:bg-white/30 flex items-center justify-center transition-all" title="Perbesar / Perkecil">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <template x-if="!modalMaximized">
                            ...
                            <button @click.stop="closeModal()"
                                class="w-7 h-7 rounded-lg bg-white/15 hover:bg-red-500 flex items-center justify-center transition-all font-black text-lg leading-none" title="Tutup">
                                &times;
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 overflow-y-auto p-6 space-y-5">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1.5">
                                Maping Kelas (Rombongan Belajar)
                            </label>
                            ...
                            <div x-show="form.jurusan && !loadingRombel && rombels.length === 0" class="p-4 rounded-xl border-2 border-dashed border-amber-200 bg-amber-50 text-sm text-amber-700 font-medium text-center">
                                    ...
                                    <label class="flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all select-none"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-xs text-gray-400">
                                                <span x-text="rombel.siswa_count"></span> siswa
                                            </p>
                                        </div>
                            ...
                            <p x-show="form.jurusan && !loadingRombel && rombels.length > 0"
                    ...
                    <div class="shrink-0 px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between gap-3">
                        ...
                        <button @click="submitForm()" :disabled="submitting"
                            ...
                            <span x-text="editMode ? (submitting ? 'Menyimpan...' : '💾 Simpan Perubahan') : (submitting ? 'Menambahkan...' : '➕ Tambah Ujian')"></span>
                        </button>
                    </div>
                </div>
            </div>
    ...
    function ukkPage() {
        return {
            ...
            toggleRombel(id) {
                ...
                if (idx > -1) {
                    this.form.rombel_ids.splice(idx, 1);
                } else {
                    this.form.rombel_ids.push(id);
                }
            },
            ...
            async onJurusanChange() {
                ...
                this.rombels = [];
                try {
                    const url = '{{ route("kaprodi.ukk.rombel-by-jurusan") }}?jurusan=' + encodeURIComponent(this.form.jurusan);
            ...
            async submitForm() {
                ...
                const url = this.editMode
                    ? `/kaprodi/ukk/set-ukk/${this.editId}`
                    : '{{ route("kaprodi.ukk.store") }}';
                ...
                try {
                    ...
                    if (!res.ok) {
                        if (res.status === 422 && data.errors) {
                            ...
                            this.toast('Periksa kembali form isian.', 'warning');
                        } else {
