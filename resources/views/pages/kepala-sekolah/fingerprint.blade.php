<x-app-layout>
    ...
    <div class="space-y-6 px-4 py-6 sm:px-6 lg:px-8" x-data="{
        ...
        get filtered() { return this.rows.filter(r => r.name.toLowerCase().includes(this.search.toLowerCase()) && (this.status === 'all' || (this.status === 'present' && r.check_in) || (this.status === 'out' && r.check_out) || (this.status === 'absent' && r.status === 'Tidak Hadir') || (this.status === 'late' && r.status === 'Terlambat') || (this.status === 'pending' && r.status === 'Menunggu Absensi'))); },
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
            @foreach(['all' => ['Total Guru', $summary['total']], 'present' => ['Total Kehadiran', $summary['present']], 'out' => ['Tercatat Pulang', $summary['out']], 'late' => ['Terlambat', $summary['late']], 'absent' => ['Tidak Hadir', $summary['absent']], 'pending' => ['Menunggu', $summary['pending']]] as $key => [$label, $value])
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <section class="rounded-2xl border bg-white p-6">
                ...
                <p class="mt-2 text-center text-xs text-gray-500">{{ $summary['absent'] }} tidak hadir setelah batas datang · {{ $summary['pending'] }} masih menunggu batas.</p>
        ...
        <p class="rounded-xl bg-blue-50 p-4 text-xs leading-relaxed text-blue-800">Sebelum batas akhir datang, pegawai tanpa scan berstatus Menunggu Absensi. Setelah batas terlewati status berubah menjadi Tidak Hadir. Jika scan susulan kemudian masuk melewati batas, status otomatis menjadi Terlambat. Tetap/full-time mengikuti Konfigurasi Waktu Absensi; part-time mengikuti awal jadwal mengajar. Hadir menggunakan scan pertama dan pulang menggunakan scan terakhir yang berbeda.</p>
    </div>
</x-app-layout>
